...
public class WarnockX extends PApplet {
...
public void setup() {
  ...
  scene.camera().setPosition(new Vec(37.5f, 37.5f, -65));
  scene.camera().setOrientation(3.1416f,0);
  /*
  ancho=150;
  profundidad=0;
  altura=150;
  */
  ...
  n= random(3,max_Tri);
  allTriang= new myTriangle[PApplet.parseInt(n)];
  generateTriangles(PApplet.parseInt(n));
...
public void draw() {
  background(0);
  drawAllTriangles(PApplet.parseInt(n));
...
//----------------------------------
public void generateTriangles(int n) {
...
public void drawAllTriangles(int n){
  for (int i = 0; i < n; i++ ) {
    if(t==0){
...
public void ligne(PVector u, PVector u1) {
...
class myTriangle{
  ...
  myTriangle(float _x1,float _y1,float _x2,float _y2, float _x3, float _y3, float _z, int _back, int _border){
    this.x1=_x1;
    this.y1=_y1;
    this.x2=_x2;
    this.y2=_y2;
  ...
  public void drawTriangle(){
    ...
    beginShape(TRIANGLES);
      ...
      vertex(x3, y3, z);
    endShape();
    zcolor.add(str(back),PApplet.parseInt(z));
  };

  public void drawTrianglePlaneXY(){
    fill(back);
    noStroke();
    beginShape(TRIANGLES);
      vertex(x1, y1, z/2000);
...
public void revisar (int x1, int x2, int y1, int y2, int lim){
     int c = 0;
     boolean aux = true;
     loadPixels();
     img.loadPixels();
...
public void pintar(int x1,int x2,int y1,int y2){
   loadPixels();
   img.loadPixels();
   for (int x=x1; x<x2;++x){
      for (int y=y1; y<y2;++y){
  ...
  public void settings() {  size(375, 375,P3D); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "WarnockX" };
    if (passedArgs != null) {
